...
def parse_ssh_log_line(line: str) -> dict[str, str | bytes | int]:
    ...
    # assume data goes to end of line unless exit status is found
    data_end_index = len(line)
    ...
    if log_entry["type"] == "SH":
        if log_entry["direction"] != ">":
            raise ValueError(
                "Invalid log line format - shell log must have '>' direction"
            )
        exit_status_index = line.find("<", data_index) + 1
        if exit_status_index not in range(len(line)):
            raise ValueError(
                "Invalid log line format - missing exit status for shell log"
            )
